...
#         AF += free_vars.update(assigned_vars)
    ...
    def ff_exp(e: Expr, bound_vars: Set[str]) -> Set[str]:
        match e:
            ...
            case Prim(op, exps):
                free_vars = set()
                for exp in exps:
                    free_vars.update(ff_exp(exp, bound_vars))
                return free_vars
            case Call(e, exps):
                free_vars = set()
                free_vars.update(ff_exp(e, bound_vars))
                for exp in exps:
                    free_vars.update(ff_exp(exp, bound_vars))
                return free_vars
    ...
    def ff_stmts(stmts: List[Stmt], bound_vars: Set[str]) -> Set[str]:
        ...
        for stmt in stmts:
            match stmt:
                ...
                case If(e, s1, s2):
                    free_vars.update(ff_exp(e, bound_vars))
                    free_vars.update(ff_stmts(s1, bound_vars.copy()))
                    free_vars.update(ff_stmts(s2, bound_vars.copy()))
                case While(e, stmts):
                    free_vars.update(ff_exp(e, bound_vars))
                    free_vars.update(ff_stmts(stmts, bound_vars.copy()))
                case Return(e):
                    free_vars.update(ff_exp(e, bound_vars))
                case FunctionDef(name, params, body, return_type):
        ...
        match e:
            case x86.Callq('collect'):
                for var in live_after:
                    ...
                    if isinstance(var, x86.Var) and var.var in tuple_var_types:
...
compiler_passes = {
    ...
    'closure coversion': convert_to_closures,
